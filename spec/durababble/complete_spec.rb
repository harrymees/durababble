...
RSpec.describe "Durababble complete durable execution support", :integration do
  ...
  let(:schema) { "durababble_complete_test_#{Process.pid}_#{object_id.abs}" }
  ...
  it "implements the guarantee matrix explicitly" do
    ...
    before = Time.parse(store.workflow(workflow_id).fetch("locked_until"))
    store.heartbeat(workflow_id:, worker_id: "worker-a", lease_seconds: 120)
    after = Time.parse(store.workflow(workflow_id).fetch("locked_until"))
    ...
    outbox_id = store.enqueue_outbox(workflow_id:, topic: "email", payload: { "to" => "a@example.com" }, key: "email:1")
