...
RSpec.describe "Durababble MySQL query plans", :integration do
  let(:backend) { durababble_store_backends.find(&:mysql?) }
  let(:schema) { "#{backend.default_schema_prefix}_plan_#{Process.pid}_#{object_id.abs}" }
  ...
  it "uses indexes for the same hot store queue and wait predicates as the Yugabyte plan suite" do
    store.migrate!
    ...
    expectations.each do |name, (sql, expected_key_fragment)|
      ...
      expect(access_paths).not_to be_empty, "expected #{name} to have table access nodes: #{JSON.pretty_generate(plan)}"
      expect(access_paths).to all(satisfy { |path| path.fetch("access_type") != "ALL" }), "expected #{name} to avoid full table scans: #{JSON.pretty_generate(plan)}"
      expect(access_paths.any? { |path| path.fetch("key", "").include?(expected_key_fragment) }).to be(true), "expected #{name} to use index containing #{expected_key_fragment.inspect}, got #{access_paths.inspect}: #{JSON.pretty_generate(plan)}"
    end
  end
  ...
  def seed_mysql_plan_fixture
    ...
    1.upto(500) do |i|
      ...
      execute("INSERT INTO #{table("waits")} (id, workflow_id, position, kind, event_key, wake_at, context, status, created_at) VALUES ('timer-wait-#{i}', 'waiting-#{i}', 0, 'timer', NULL, #{mysql_literal(now - 60)}, #{wait_context}, 'pending', #{created_at})")
