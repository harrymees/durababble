...
Stable spans include `durababble.workflow.start`, `durababble.workflow.resume`, `durababble.workflow.execute`, `durababble.workflow.step`, `durababble.object.query`, `durababble.object.command.enqueue`, `durababble.object.command`, `durababble.worker.tick`, `durababble.workflow_rpc.route`, `durababble.workflow_rpc.handle`, `durababble.rpc.client.*`, `durababble.rpc.server.*`, and `durababble.store.operation`.
Stable metrics include workflow start/completion/failure counters, step attempt/success/failure/retry counters, wait start/completion counters and latency histograms, queue claim latency, lease heartbeat/conflict/recovery counters, outbox pending/processed counters, store operation duration/error metrics, worker tick duration/counts, and workflow replay/history size measurements.
