...
Workflow code is deterministic orchestration in `#execute`; side-effecting methods become durable boundaries with `step`. Completed steps replay from persisted results, and replay shape checks fail with `Durababble::NonDeterminismError` if code reaches a different persisted step method or returns before consuming persisted history. Cancellation is cooperative: `Workflow.handle(run_id).cancel(reason:)` durably records a request and the next deterministic yield point raises `Durababble::CancellationError` inside workflow code. User code can rescue it, run durable cleanup steps, and finish the run as `canceled`; cleanup failures are recorded as workflow failures.
