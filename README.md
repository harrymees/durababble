...
If replayed code reaches a different step method at a persisted position, or returns before consuming all persisted step positions, the run fails with `Durababble::NonDeterminismError` instead of silently reusing incompatible history.
