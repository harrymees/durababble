...
If `store.signal_event` records an event before a matching `wait_event` exists, Durababble stores that emission durably and delivers it to the first later wait for the same event key.
