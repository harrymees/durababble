...
Replay is intentionally strict. If deployed code reaches a different persisted step method at the same position, or returns before consuming persisted history, the run fails with `Durababble::NonDeterminismError` instead of quietly attaching old side effects to new control flow.
