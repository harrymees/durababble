...
// Fires once room.users is populated and updateUserState/emit are safe to call.
// Runs on initial join AND after every reconnect, so the same setup code works
// in both cases.
room.on("ready", () => {
  room.updateUserState({ status: "online" });
});
