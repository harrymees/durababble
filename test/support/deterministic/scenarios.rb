    ...
    module Scenarios
      ...
      def multiple_named_object_wakes(seed)
        run(seed, "multiple_named_object_wakes") do |h|
          ...
          h.check("all three wakes handled in name order") do
            state = h.store.object_state(object_type: "alarm", object_id:)
            state && state.fetch("wakes") == ["daily", "retry", "ttl"]
          end
          h.check("no pending wakes remain") { h.store.instance_variable_get(:@object_wakeups).empty? }
        end
      end
      ...
      # A durable object with several independently named wakes. `on_wake` is
      # idempotent (set union) so at-least-once redelivery cannot double-record.
      #: () -> untyped
