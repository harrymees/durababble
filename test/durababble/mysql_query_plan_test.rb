...
class DurababbleMysqlQueryPlanTest < DurababbleTestCase
  test "uses indexes for the same hot store queue and wait predicates as the Yugabyte plan suite" do
    ...
    with_durababble_store(backend, "plan") do
      ...
      expectations = {
        ...
        "workflow event wait wake probe" => [
          "SELECT w.* FROM #{table("waits")} AS w JOIN #{table("workflows")} AS wf ON wf.id = w.workflow_id WHERE w.status = 'pending' AND w.scope = 'workflow' AND wf.status IN ('waiting', 'running') AND kind = 'event' AND event_key = 'target-event' FOR UPDATE SKIP LOCKED",
          "waits_event_pending",
        ],
        "workflow timer wait wake probe" => [
          "SELECT w.* FROM #{table("waits")} AS w JOIN #{table("workflows")} AS wf ON wf.id = w.workflow_id WHERE w.status = 'pending' AND w.scope = 'workflow' AND wf.status IN ('waiting', 'running') AND kind = 'timer' AND wake_at <= NOW(6) FOR UPDATE SKIP LOCKED",
          "waits_timer_pending",
        ],
      }
  ...
  def seed_mysql_plan_fixture
    ...
    1.upto(500) do |i|
      ...
      execute("INSERT INTO #{table("waits")} (id, workflow_id, position, scope, kind, event_key, wake_at, context, status, created_at) VALUES ('timer-wait-#{i}', 'waiting-#{i}', 0, 'workflow', 'timer', NULL, #{mysql_literal(timer_wake_at)}, #{wait_context}, 'pending', #{created_at})")
      event_key = i == 1 ? "target-event" : "other-event"
      execute("INSERT INTO #{table("waits")} (id, workflow_id, position, scope, kind, event_key, wake_at, context, status, created_at) VALUES ('event-wait-#{i}', 'waiting-#{i}', 0, 'workflow', 'event', #{mysql_literal(event_key)}, NULL, #{wait_context}, 'pending', #{created_at})")
