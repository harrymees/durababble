  ...
  durababble_store_backends.each do |backend|
    test "terminates a running workflow without delivering cancellation cleanup with #{backend.name}" do
      with_durababble_store(backend, "workflow_termination_running") do |store|
        ...
        assert_equal(["canceled"], steps.map { |step| step.fetch("status") })
        assert_equal(["operator hard stop"], steps.map { |step| step.fetch("error") })
        assert_equal(["canceled"], attempts.map { |attempt| attempt.fetch("status") })
        assert_equal(["operator hard stop"], attempts.map { |attempt| attempt.fetch("error") })
