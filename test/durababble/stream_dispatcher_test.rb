...
# a live transport: the `expose_stream` DSL registration and the server-side
# `StreamDispatcher` driven against a fake store + capturing writer.
class DurababbleStreamDispatcherTest < DurababbleTestCase
  ...
  test "object streams run with exposed-query mutation guards" do
    starter_class = Class.new(Durababble::Workflow) do
      workflow_name "object_stream_forbidden_start"
    end
    mutation_store = Object.new
    mutation_store.define_singleton_method(:enqueue_workflow) do |**_kwargs|
      raise "object stream escaped the exposed-query context"
    end
    object_class = Class.new(Durababble::DurableObject) do
      const_set(:STARTER, starter_class)
      const_set(:MUTATION_STORE, mutation_store)
      object_type "dispatcher_object_stream_query_context"
      expose_stream :unsafe
      def unsafe
        self.class::STARTER.enqueue({}, store: self.class::MUTATION_STORE, worker_pool: "default")
      end
    end
    request = Durababble::Rpc::Messages::TransientRequest.new(
      class_name: "dispatcher_object_stream_query_context",
      durable_object_id: "unsafe-1",
      method: "unsafe",
    )
    writer = CapturingWriter.new
    ...
    assert_match(/cannot start workflows from an exposed query/, error.message)
  ...
  test "workflow streams run with exposed-query mutation guards" do
    starter_class = Class.new(Durababble::Workflow) do
      workflow_name "workflow_stream_forbidden_start"
    end
    mutation_store = Object.new
    mutation_store.define_singleton_method(:enqueue_workflow) do |**_kwargs|
      raise "workflow stream escaped the exposed-query context"
    end
    workflow_class = Class.new(Durababble::Workflow) do
      const_set(:STARTER, starter_class)
      const_set(:MUTATION_STORE, mutation_store)
      workflow_name "dispatcher_workflow_stream_query_context"
      expose_stream :unsafe
      def unsafe
        self.class::STARTER.enqueue({}, store: self.class::MUTATION_STORE, worker_pool: "default")
      end
    ...
    request = Durababble::Rpc::Messages::TransientRequest.new(
      workflow_id: "wf-1",
      class_name: "dispatcher_workflow_stream_query_context",
      method: "unsafe",
    )
    writer = CapturingWriter.new
