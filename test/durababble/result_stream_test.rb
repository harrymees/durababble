...
class DurababbleResultStreamTest < DurababbleTestCase
  ...
  test "close after a reactor read can be called again off reactor" do
    stream = nil
    ...
    # The transient producer was cancelled when the reactor scope ended. A later
    # close from normal synchronous code should only mark the queue closed; there
    # is no ambient reactor left to wait on.
    stream.close
    stream.close
  end
