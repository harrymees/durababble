...
Durababble workers form a peer-to-peer gRPC mesh over HTTP/2. When code somewhere in the cluster calls `handle.label` on a workflow handle or reads `account.balance` on a durable object reference, the call is delivered straight to the pod that currently holds the lease for that entity — no database round-trip in the request path. This page explains why that mesh exists, what it is and isn't, and how it stays cheap.
...
- `NodeUnavailable` — the peer is unreachable (connection refused, TCP timeout, HTTP/2 stream reset, gRPC `Unavailable`, gRPC `DeadlineExceeded`, or gRPC `Cancelled`). The lease is re-read and the new owner is dialed.
...
**Unexpected raises on the peer are not retried.** When a handler raises something the protocol does not recognise, the server returns gRPC `Internal` and the client surfaces it as `Rpc::Error` (which is deliberately _not_ a subclass of `Rpc::Unavailable`). The router lets it propagate. This is on purpose: a bug that takes one peer's handler down should not be amplified into a stampede that takes the rest of the cluster with it. If the failure is genuinely transient, the caller (or the durable command machinery, for `expose_command` calls) gets to decide whether to retry; the RPC layer does not retry blindly.

The gRPC status mapping the transport uses:

| Status | Client raises | Retried by router? |
| --- | --- | --- |
| OK | (success — payload returned) | — |
| Unauthenticated | `Rpc::Unauthenticated` | no |
| Internal | `Rpc::Error` | **no** (handler bug) |
| Unavailable / DeadlineExceeded / Cancelled | `Rpc::Unavailable` → `NodeUnavailable` | **yes** (peer down) |
| other | `Rpc::Error` | no |

Unavailable-style statuses are reserved for transport-level unavailability and are currently produced by the client's `with_rpc_errors` translation of socket / HTTP/2 / gRPC deadline errors, not by the server's request handler. A peer that reaches the handler and raises always gets `Internal`.

## What This Buys

- **No write amplification.** Status reads, live counters, and warm-path RPCs do not write to SQL.
- **Latency is gRPC-over-HTTP/2 latency.** Calls land on the lease holder directly; there is no orchestration hop.
...
The intended hardening path is to layer real authenticated transport — mTLS at minimum, ideally SPIFFE-style workload identity — under the existing gRPC transport. In Shopify-style deployments this is typically provided by the infrastructure (sidecar mTLS, service-mesh identity) rather than by Durababble itself, so the gem's job is to be _compatible_ with that, not to ship its own crypto. A future version will document the expected sidecar / mesh setup and may grow a built-in TLS configuration for deployments that don't have one of those.
...
The gRPC transport, peer routing, lease-keyed addressing, stale-lease retries, and the no-retry-on-unexpected-raise policy described above are implemented and exercised by the test matrix. Admin surfaces, richer health and routing observability for the mesh, and the transport-security hardening above are target work — see [the reference](reference.md) for the current prototype boundary.
