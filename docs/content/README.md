...
Workflow orchestration can also call workflow and durable-object handles directly, and can start child workflows with replay-safe handles; those in-workflow calls are persisted as workflow history commands so replay and crash recovery do not send the outbound RPC or child start twice.
