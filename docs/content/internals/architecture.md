...
Exposed object queries are transient owner-local reads. If an active object lease exists, callers route the query through `CallTransient` to that owner, or take an in-process fast path only when the current runtime owns the same lease. If no active owner exists, a caller must claim the object lease before reading persisted state. In both cases the read gate rejects the query with `Durababble::ObjectReadBlocked` while pending, running, retry-failed, or dead-lettered mailbox work exists, so a caller does not observe a stale snapshot that has skipped unresolved commands.
