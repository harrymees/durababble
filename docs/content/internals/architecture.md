...
- `workflows`: one row per run; stores status, input, result, errors, workflow lease owner/deadline, `next_run_at` for durably scheduled step retries and earliest unresolved workflow timers, first cooperative cancellation request metadata (`cancel_reason`, `cancel_requested_at`, `cancel_delivered_at`), nullable child parent/origin/cancellation metadata, a nullable `colocation_group_id` for children bound to a colocation group, and the terminal `terminated` state for operator hard stops.
...
- `durable_objects`: latest durable-object state by `(worker_pool, object_type, object_id)`, plus a nullable `colocation_group_id` for objects bound to a colocation group.
- `colocation_groups`: one row per colocation group, identified by a deterministic group id (`wf:<parent_workflow_id>` for a workflow parent, `obj:<object_type>/<object_id>` for an object parent). Each row is a single coordination mutex carrying the group's current lease owner and deadline, so it answers "who, if anyone, currently holds this placement group" in one row.
...
- Colocation (`colocate: true` on a child workflow start or `DurableObject#start_workflow`) is a worker placement constraint layered on top of normal durability. At start time the child inherits the parent's existing `colocation_group_id` if it has one, otherwise the group id is derived deterministically from the parent identity (`wf:<parent_workflow_id>` or `obj:<object_type>/<object_id>`), and both parent and child are stamped with that id first-writer-wins. The flag is folded into the child's idempotency identity, so retrying a start with a different `colocate:` value raises `IdempotencyKeyConflict` rather than silently changing placement.
- A colocation group holds at most one live lease holder at a time. The `colocation_groups` row carries a lease owner and deadline that is independent of any individual member lease but, for colocated starts, is acquired for the parent's current owner and given the parent's lease window so the two expire together. While that group lease is live and owned by another worker, no other worker can claim any member of the group; this is what keeps a colocated parent from being stolen while its child is still running, and vice versa.
- Colocation is crash safe because the group lease can lapse like any other. If the holder's lease expires or its process disappears, the group has no live holder and is free. Once free, any worker may claim the parent, the child, or both — but the same claim statement that leases a member also acquires (or renews) the group lease, so a worker that takes one live member takes the whole group. Two members can never be actively leased by two different workers; if both are leased they are leased together. Durababble never pins a member to one worker permanently, so a dead worker does not strand colocated work.
- Colocation adds no queries to the lease, poll, or task-write hot paths. On PostgreSQL the co-tenancy check and group lease acquisition are folded into the existing single claim statement as data-modifying CTEs: a candidate CTE excludes a member whose group is held by a live foreign lease, a group CTE acquires or renews the lease when the group is free, expired, or already ours, and the member `UPDATE` only lands when the group is null or the group CTE took it. A blocked claim returns an empty row set and the worker simply moves on. MySQL and SQLite reach the same outcome by locking the candidate row `FOR UPDATE` and acquiring the group inside the same claim transaction. Non-colocated members short-circuit entirely, so workloads that never opt in pay nothing.
