...
- `workflows`: one row per run; stores status, input, result, errors, workflow lease owner/deadline, `next_run_at` for durably scheduled step retries and earliest unresolved workflow timers, first cooperative cancellation request metadata (`cancel_reason`, `cancel_requested_at`, `cancel_delivered_at`), nullable child parent/origin/cancellation metadata, nullable colocation owner columns (`colocated_owner_object_type`/`colocated_owner_object_id`) naming the object whose lease a colocated child is bound to, and the terminal `terminated` state for operator hard stops.
...
- `durable_objects`: latest durable-object state by `(worker_pool, object_type, object_id)`, including the object's own lease owner/deadline (`locked_by`/`locked_until`), plus nullable colocation owner columns (`colocated_owner_object_type`/`colocated_owner_object_id`) for an object that is itself a colocated child of another object. There is no separate group table — a colocated child points directly at its root owner object, and that object's own lease is the single coordination mutex for the whole colocated set.
...
- Colocation (`colocate: true` on `DurableObject#start_workflow` or `DurableObject#start_object`) is a worker placement constraint layered on top of normal durability, and is supported only from an object command — workflow-to-workflow colocation does not exist. At start time the child records its root owner object in `colocated_owner_object_type`/`colocated_owner_object_id`: if the starting object is itself colocated the child inherits that object's owner (flattening to the root), otherwise the owner is the starting object itself. Every colocated child therefore points directly at the root object, so no transitive resolution is needed at claim time. The flag is folded into the child's idempotency identity, so retrying a start with a different `colocate:` value raises `IdempotencyKeyConflict` rather than silently changing placement.
- The owner object's own lease (`durable_objects.locked_by`/`locked_until`) is the single master lease for the colocated set — there is no separate group lease. To claim a colocated child (workflow or object) a worker must already hold, or atomically acquire, the owner object's lease. A child with a `NULL` owner is ordinary work and skips the gate entirely. While the owner object's lease is live and held by another worker, no other worker can claim any colocated child of it; this is what keeps a colocated child from being stolen while the object runs, and vice versa.
- The fence survives crashes. A colocated child workflow's step heartbeat also renews the owner object's lease, so a long-running child keeps the object protected from takeover even though the object's command finished long ago. The owner object's lease is released only by a guarded release: `release_object_lease` clears the lease only when no colocated child is still live (no non-terminal colocated child workflow, and no colocated child object whose own lease is currently held). Because you cannot claim a child without holding the owner, and cannot release the owner while any child is live, the owner and its children can never be actively leased by two different workers. If the owner's worker dies, its lease and the children's leases all lapse; once everything is idle the guard passes and the next claim re-homes the object and its children together onto one worker. Durababble never pins work to one worker permanently, so a dead worker does not strand colocated work.
- Colocation is cheap. Non-colocated work has `NULL` owner columns and short-circuits every colocation check on the claim, poll, heartbeat, and task-write paths, paying nothing. On PostgreSQL a colocated claim folds the owner-object lease acquisition into the existing single claim statement as a data-modifying CTE and gates the candidate on the owner row, so it adds no extra round trip; MySQL and SQLite lock the candidate row `FOR UPDATE` and acquire the owner lease inside the same claim transaction. The only added cost is on the cold release path: one indexed `NOT EXISTS` probe for live colocated children, which is empty for non-colocated objects. No group table, group-row upsert, or group steal/release sweep exists, so the redesign is strictly cheaper than a separate group lease would be.
