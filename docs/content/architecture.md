...
- `formal/workflow_storage.als`: Alloy model for workflow state, ordered workflow command history, leases, timer waits, fences, outbox rows, target activations, and FIFO inbox command serialization for object and workflow targets. `scripts/verify-alloy.sh` verifies the model and `scripts/validate-durababble-sigils.rb` keeps `[DURABABBLE-*]` obligations synchronized with Ruby implementation/tests.
...
The high-risk durability and lease claims above are also modeled in Alloy. The CI-gated command is `mise exec -- bundle exec rake formal`, which runs Alloy verification plus the Durababble sigil validator.
