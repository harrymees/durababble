...
Durababble enforces its payload limits after Paquito serialization and before writing or sending the serialized bytes. The default for `max_workflow_input_bytes`, `max_workflow_result_bytes`, `max_step_output_bytes`, `max_object_state_bytes`, `max_inbox_payload_bytes`, and `max_rpc_argument_bytes` is 4 MiB each, with `max_workflow_args_bytes` retained as a compatibility alias for workflow inputs. These checks are deliberately below the storage column capacities: MySQL/MariaDB `LONGBLOB` can store far larger values than the Durababble default, and PostgreSQL/YSQL `bytea` is likewise not the intended application-level bound. The Durababble limits are operational guardrails for replay cost, RPC pressure, and accidental payload capture, not a substitute for backend column definitions.
