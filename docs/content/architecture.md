...
- `Durababble::ChildWorkflowHandle`: explicit handle returned by workflow `start_child` / `start_workflow` and durable-object `start_workflow`. It exposes child workflow id, worker pool, cancellation policy, status/result/error, `await`, `cancel`, `terminate`, and `ref`, while the store owns replay-safe linkage and latest observed outcome.
...
- `formal/workflow_storage.als`: Alloy model for workflow state, ordered workflow command history, leases, timer waits, fences, outbox rows, target activations, and FIFO inbox command serialization for object and workflow targets. `scripts/verify-alloy.sh` verifies the model, and `test/durababble/formal_sigil_drift_test.rb` keeps `[DURABABBLE-*]` obligations synchronized with Ruby implementation/tests on every PR.
...
Direct waits, child workflow operations, and in-workflow handle RPCs use the same monotonic command ids and replay validation as steps, but their command names are the wait helper, `child_workflow:<workflow_name>:start`, `child_workflow:workflow:observe`, or `handle_rpc:<target_kind>:<target_type>:<method>` rather than user step method names. A direct wait records a scheduled command before the wait row is inserted, records a waiting history entry when the wait is persisted, and later consumes the completed timer payload to resume the blocked workflow fiber. A child start records a scheduled command before inserting the child workflow row and link row; a child observe records the child's current status/result/error as the command resolution. A handle RPC records a scheduled command before dispatching the workflow/object handle call and records the returned result or error as the command resolution, so replay can reuse the recorded result without sending the outbound inbox message or query again. Replay indexes terminal history by command id and uses the latest terminal event, so a completed timer wake overrides the earlier waiting event for the same command.

Workflow code starts children with `start_child(ChildWorkflow, input, id: nil, worker_pool: nil, idempotency_key: nil, cancellation: :request_cancel)` or `start_workflow(...)`. The returned `ChildWorkflowHandle#await` polls through recorded observe commands and durable timer waits, so a parent crash after child creation or while waiting does not duplicate the child and does not lose the wait. Child workflows run in their persisted worker pool; a child in another pool or without a registered worker remains pending and inspectable until a matching worker appears or an operator acts on the handle. Child retry is independent from parent retry.
...
Durable object commands can start workflows with `start_workflow(WorkflowClass, input, id: nil, worker_pool: nil, idempotency_key: nil, cancellation: :abandon)`. The object command persists an object-origin `child_workflows` link immediately using the durable command id, so a retry of the same command reattaches to the child instead of starting another one. Object commands do not synchronously wait for children; they observe later by storing the child id in object state, scheduling a named wake, receiving a later signal/command, or reading a workflow handle after the command commits.
...
- `child_workflows`: workflow-origin and object-origin child links keyed by a stable link id, with origin metadata, child workflow id/name, worker pool, idempotency key, cancellation policy, serialized input, latest observed status/result/error, and completion timestamp for inspection.
...
- Child workflow starts commit the child workflow row and `child_workflows` link row atomically before returning a handle. Parent replay reuses the recorded start result, child observe commands reuse recorded status/result/error, and `await` parks through normal durable waits while the child is non-terminal. Parent cancellation requests child cancellation only for links with `cancellation_policy = 'request_cancel'`; parent hard termination does not run cleanup and does not mutate linked children.
