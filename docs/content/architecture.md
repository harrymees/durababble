...
- Enqueue persists the workflow before work is claimed. Callers may provide an explicit workflow id; the store inserts that id through the workflow row primary key and raises `Durababble::WorkflowAlreadyExists` on duplicates before any workflow history, wait, inbox, or target activation side effects are written.
