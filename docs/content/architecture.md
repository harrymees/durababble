...
Direct waits use the same monotonic command ids and replay validation as steps, but their command names are the wait helpers rather than user step method names. A direct wait records a scheduled command before the wait row is inserted, records a waiting history entry when the wait is persisted, and later consumes the completed timer payload to resume the blocked workflow fiber. Replay indexes terminal history by command id and uses the latest terminal event, so a completed timer wake overrides the earlier waiting event for the same command.
