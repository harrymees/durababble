...
- `formal/workflow_storage.als`: Alloy model for workflow state, ordered workflow command history, leases, timer waits, fences, outbox rows, target activations, and durable-object FIFO command serialization. `scripts/verify-alloy.sh` verifies the model and `scripts/validate-durababble-sigils.rb` keeps `[DURABABBLE-*]` obligations synchronized with Ruby implementation/tests.
