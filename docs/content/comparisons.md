...
- **Replay model.** Both persist step return values and skip completed steps on resume, so neither requires Temporal-style deterministic replay of all branch logic. Durababble does still require deterministic ordering around step calls and raises `ReplayDivergenceError` on divergence.
