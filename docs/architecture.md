...
Async workflow code keeps that same identity rule. `async(:expensive_step, input)` validates a registered step method, reserves the next step position immediately, and returns a future. `future.start`, `future.value`, and `await_all(futures)` run the reserved step in Ruby threads, but the durable step call consumes the reserved position instead of racing on the shared step cursor. `await_all` starts all pending futures, waits for every started future to settle, then returns results in the order the caller supplied the futures. If any future failed, the first failure by supplied order is raised after siblings have settled, so replay does not depend on wall-clock completion order.

The block form, `async { expensive_step(input) }`, is still available for advanced experiments but is intentionally narrow: it may contain one durable boundary, usually one step method. It is not a general deterministic fiber system. Cancellation before start persists a `canceled` step/attempt row at the reserved position; cancellation after a side-effecting step starts is best-effort and returns false.
