...
- `Durababble::Engine`: creates/resumes workflow runs, enforces workflow lease ownership, records workflow step transitions, handles explicit step heartbeats, handles retryable step failures, handles workflow-level and compatible step-scoped waits, and skips completed durable history during recovery.
...
- `waits`: durable timer and external-event waits, including context needed to resume. The `scope` column distinguishes workflow-level wait history from compatible step-scoped waits.
...
- Workflow-level wait requests persist a `waits` row with `scope='workflow'`, clear the workflow lease, and put the workflow in `waiting` until timer wake or event signal completes the wait history and makes the workflow pending again. Compatible step-scoped wait requests use `scope='step'` and additionally update the waiting step and latest attempt.
