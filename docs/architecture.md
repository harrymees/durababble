...
- `sig/durababble.rbs`: static-only RBS declarations for the public class API. Runtime execution does not load or validate RBS. The local `rake typecheck` path combines `rbs validate`, a strict public-signature gate, and Sorbet inline comment checking so signature drift and dynamic escape annotations fail before CI handoff.
