...
- `Durababble::Engine`: creates/resumes workflow runs, enforces workflow lease ownership, records workflow step transitions, handles explicit step heartbeats, handles retryable step failures, handles root workflow-level waits, and skips completed durable history during recovery.
...
Workflow-level waits use a separate deterministic wait position stream in `waits`, so calling `wait_until` or `wait_event` from the root `#execute` path parks the workflow without creating a side-effect step row. Wait helpers raise outside workflow execution, inside durable steps, and inside child Async branches because those contexts cannot safely suspend the whole workflow. If replay reaches a completed step position with a different current method name, reaches a completed workflow-level wait with an incompatible wait shape, or returns before all completed step or workflow wait positions have been consumed, the engine fails the run with `Durababble::NonDeterminismError` so code changes cannot silently reuse stale results or drop recorded durable history.
...
- `waits`: durable workflow-level timer and external-event waits, including context needed to resume. The `scope` column is retained for schema compatibility, but supported execution only writes and wakes `workflow` rows.
...
- On resume, only `completed` steps are skipped; incomplete/running/failed work is retried or continued. For a retried step, `step_context.heartbeat.cursor` exposes the latest cursor from the previous incomplete invocation.
- Workflow-level wait requests persist a `waits` row with `scope='workflow'`, clear the workflow lease, and put the workflow in `waiting` until timer wake or event signal completes the wait history and makes the workflow pending again.
