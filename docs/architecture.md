...
- The benchmark operation set intentionally covers the main prototype lifecycle: enqueue, claim, heartbeat, lease conflict/recovery, worker tick/drain, end-to-end event and timer waits, resume-with-completed-steps, failed-workflow retry, observability reads, idempotency fences, outbox claim/ack/reclaim, durable-object command claim/complete/read, large-table query shapes, and cross-process command RPC.
...
- Store migrations create queue/recovery indexes for workflow claims, expired leases, worker lease release, pending event waits, due timers, cancellation wait cleanup, outbox delivery scans, and durable-object command scans so the benchmark suite exercises production-intended query plans rather than relying on tiny-table behavior.
