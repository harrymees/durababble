...
- **Replay shape checks.** A completed step is only replayed when the current workflow reaches the same durable position with the same recorded step method name. A mismatch fails the workflow with `Durababble::NonDeterminismError` instead of silently returning a stale result for different code.
...
SimpleCov thresholds currently required by the suite:
