...
- **Operator UI.** `docs/operator-web-ui.md` specifies the target operator-facing web UI for workflow/object inspection, progress views, lease/outbox/wait diagnostics, and management actions such as cancel, terminate, retry/resume, and pause/resume. The current prototype has many underlying durable rows and some detail reads, but the UI and most management APIs remain follow-up work.
