...
- **Timer waits.** `Durababble.wait_until` persists workflow-level timer waits when called directly from `#execute`, and persists compatible step-scoped waits when returned by a step method. Both shapes resume workflows after the wake time.
- **External event waits.** `Durababble.wait_event` and `Store#signal_event` persist event waits and wake matching workflows. Direct calls from `#execute` are workflow-level wait yield points; step-returned waits remain compatible.
...
| `waits`               | durable workflow-level and compatible step-scoped timer/event waits                           | wait id, workflow id, scope, position, due time/event key/status |
