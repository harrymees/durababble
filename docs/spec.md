...
Child-workflow orchestration was designed against Temporal Ruby child workflows ([docs.temporal.io/develop/ruby/workflows/child-workflows](https://docs.temporal.io/develop/ruby/workflows/child-workflows), [ruby.temporal.io/Temporalio/Workflow.html](https://ruby.temporal.io/Temporalio/Workflow.html), observed SDK HEAD `205e9a153751afae5a4dcf0e39a0a1a95e6afc91`), Hatchet child spawning ([docs.hatchet.run/v1/child-spawning](https://docs.hatchet.run/v1/child-spawning), observed repo HEAD `afbde7cf8f9f4e5bdf31b015569909693f8c5949`), and Absurd's Postgres-native checkpoint/await/retry model ([earendil-works.github.io/absurd](https://earendil-works.github.io/absurd/), observed repo HEAD `f2fcc45db4dfa46cd44cab36a4aa1f5d9e393bbd`). Temporal records child lifecycle in parent history and exposes start-plus-handle vs execute-and-await APIs with parent close policy; Hatchet treats child spawning as durable task/workflow fanout across the worker fleet; Absurd emphasizes checkpointed tasks, sleeps, event awaits, and retry from the last checkpoint rather than a named child-workflow surface. Durababble follows the smaller Ruby-friendly model: an explicit child handle, SQL parent/child link rows, replay-safe starts/observes, independent child retry, and an explicit cancellation policy instead of implicit parent-close cleanup.

Workflow code starts a child with `start_child(workflow_class, input, id: nil, worker_pool: nil, idempotency_key: nil, cancellation: :request_cancel)` or the alias `start_workflow(...)`. The call returns `Durababble::ChildWorkflowHandle`, whose public surface is `workflow_id`, `worker_pool`, `cancellation_policy`, `status`, `result`, `error`, `await(poll_interval: 1, timeout: nil)`, `cancel(reason: nil)`, `terminate(reason: nil)`, and `ref`. `await` returns the child's result when it completes, raises `Durababble::ChildWorkflowFailed`, `Durababble::ChildWorkflowCanceled`, or `Durababble::ChildWorkflowTerminated` for terminal non-success outcomes, and raises `Durababble::CommandTimeout` if an explicit timeout expires.

Child start is a durable workflow command. Durababble records the parent command schedule before inserting the child workflow row and `child_workflows` link row in one store transaction; if the parent crashes after the child is created, replay reuses the recorded command result and reattaches to the existing child instead of inserting another child. When `id:` is omitted, the child id is generated from parent workflow id, command id, child workflow name, input, worker pool, and idempotency key. When `idempotency_key:` is omitted, it is generated from parent workflow id plus command id. Reusing the same durable parent command or same object command reattaches to the existing link; reusing a key with a different child name, child id, input, worker pool, or cancellation policy raises `Durababble::IdempotencyKeyConflict`.

Awaiting or observing a child is also a durable workflow command. `status`, `result`, `error`, and each `await` poll record an observe command whose completed result is reused on replay. If the child is still pending/running/canceling, `await` records a timer wait and suspends the parent at a normal workflow safe point; a crash while waiting replays the existing child handle and continues polling until the child reaches a terminal state. Completed child results are cached through the `child_workflows` link row and through the parent's command history, but the child workflow row remains the source of truth.
...
Durable object commands may start workflows with `start_workflow(workflow_class, input, id: nil, worker_pool: nil, idempotency_key: nil, cancellation: :abandon)`. This API is command-only: exposed queries and code outside an object command cannot start workflows because that would mutate durable state without a mailbox command boundary. The start persists immediately through the same `child_workflows` link model as workflow-origin children, using `origin_kind = "object"`, the object type/id, the durable object command id, and a generated idempotency key derived from `command_context.idempotency_key`. A retry of the same object command reattaches to the existing child. Durable object commands do not synchronously `await` child workflows; they observe child outcomes safely by storing the child id in object state, scheduling a named wake, receiving a later workflow command/signal, or exposing a query that reads `Workflow.handle(child_id).status/result` after the command has committed.
...
- Child workflow starts require an explicit child-cancellation policy in durable state. Parent cancellation requests child cancellation only for linked non-terminal children whose policy is `request_cancel`; links with `abandon` keep running independently. Parent cancellation never hard-terminates children and never claims child cleanup completed.
...
| `child_workflows` | Durable parent/object-to-child workflow linkage, latest observed child outcome, idempotency, and cancellation policy | Link id, parent workflow/command, parent object/command, child workflow id, parent/object listing |
...
- Child workflow link rows are keyed by a stable generated link id. Workflow-origin links are unique by `(parent_workflow_id, parent_command_id)`, object-origin links are unique by `(parent_object_type, parent_object_id, parent_object_command_id, idempotency_key)`, and listing indexes cover parent workflow, parent object, and child workflow id lookup.
...
| After child workflow start commits, before parent awaits | Parent replay reattaches to the existing child link and does not create a duplicate child workflow. |
