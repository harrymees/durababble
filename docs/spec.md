...
`colocate: true` (default `false`) asks Durababble to keep the child and its parent on the same worker: at start time the child is bound to the parent's colocation group, so the parent's current owner is the only worker that may hold both leases at once. Colocation is a co-tenancy constraint, not a permanent pinning — see [Colocation](#colocation). Colocation does not override worker pools; a colocated child still runs in its resolved pool, so `colocate: true` is meaningful only when the child and parent share a pool that some worker serves. Object-origin child starts accept the same flag and bind the child to the originating object's colocation group. `colocate` is part of the child-start shape: reusing the same durable parent command with a different `colocate` value raises `Durababble::IdempotencyKeyConflict`, exactly like a child name or input mismatch.
...
| `colocation_groups` | Single mutex row per colocated parent/child group, gating co-tenant member leases to one worker | Group id, owner worker identity, lease expiry |
...
- Colocation adds a nullable `colocation_group_id` to `workflows` and `durable_objects` plus a `colocation_groups` mutex table keyed by group id. The column is `NULL` for the overwhelmingly common non-colocated case, and every claim/heartbeat path short-circuits the group check on `NULL` so non-colocated work pays nothing. Claim paths acquire the group lease in the same atomic statement that acquires the member lease (a data-modifying CTE on PostgreSQL/YSQL; a conditional group `UPDATE` between candidate select and member update on MySQL/MariaDB), gated so a group held live by another worker is not stolen and group-blocked candidates are skipped. Worker-lease release and expired-lease steal sweeps clear matching `colocation_groups` rows with one extra pool-wide `UPDATE`, so colocated work re-homes as a unit without forcing a permanent worker pinning.
...
Sticky placement is a best-effort routing optimization. Colocation is a durable correctness constraint: when a child workflow or object-origin child is started with `colocate: true`, the child and its parent must never be actively leased by two different workers at the same time. The runtime models this with a colocation group — a single mutex row that the members' leases are gated on.

- **Deterministic group identity.** A colocation group id is derived from the parent target identity: `wf:<parent_workflow_id>` for workflow parents and `obj:<object_type>/<object_id>` for object parents. Deriving the id (rather than allocating one) makes group binding replay-safe and idempotent: a parent that retries a child-start command computes the same group id and rebinds to the same row.
- **Transitive membership.** Starting a colocated child stamps the child's `colocation_group_id` with the parent's group. If the parent is not yet in a group, the parent's own row is stamped with its derived group id at the same time, so the parent and child join one group. A colocated grandchild inherits its (already-grouped) parent's group id, so an entire colocated tree forms one group that lives on one worker.
- **Group co-tenancy invariant.** A colocation group has at most one live lease holder at a time. A worker may claim a member (workflow or object) only if the group row is unleased, expired, or already owned by that worker; acquiring the member lease and the group lease is one atomic step. While the group is live on worker A, no other worker can claim any member of the group — this is what prevents a second worker from leasing the parent while the colocated child is still running on A.
- **Group binding at start.** When a colocated child is created, the group row is upserted to the parent's current owner with a deadline tied to the parent's lease window. The parent is the live owner at start time, so this hands the group to the worker that is already running the parent. The upsert only takes ownership when the group is free, expired, or already the parent's worker, so it is safe under replay and concurrent recovery.
- **Group keepalive on claim and heartbeat.** Claiming or heartbeating a colocated member also refreshes the group deadline, so a long-running colocated step keeps the whole group — including a parent whose own lease has lapsed — protected from takeover. Non-colocated members never touch the group row.
- **Crash-safe re-homing.** Colocation never pins a target to one worker forever. If a worker dies, the member leases and the group lease all expire; recovery sweeps clear them, and the next claim re-homes the whole group onto a new worker atomically. A new worker may then lease the parent, the child, or both — but if both are active, they are always co-tenant on the same worker.
- **Hot-path cheapness.** Non-colocated targets have `colocation_group_id IS NULL` and short-circuit every group check, paying nothing on the claim, poll, heartbeat, or task-write paths. Colocated claims fold group acquisition into the existing single claim statement (a CTE on PostgreSQL/YSQL, a conditional group update wedged between the candidate select and the row update on MySQL/MariaDB) and so add no extra round trip. The claim candidate filter also skips rows whose group is held live by another worker, so group-blocked work never stalls a polling worker. Group cleanup rides on the existing worker-lease-release and expired-lease-steal sweeps with a single extra `UPDATE`.
...
| Colocated members share one live worker | A colocation group has at most one live lease holder; a worker claims or heartbeats a colocated member only while acquiring/holding the group lease, so parent and colocated child are never actively leased by two workers at once. | Colocation claim-gate and parent-fence backend conformance specs plus child-start reuse conflict specs |
| Colocation is crash-safe, not permanent pinning | When the holder dies, member and group leases expire and recovery re-homes the whole group atomically onto the next worker; colocation never strands work on a dead worker. | Colocation crash/recovery specs plus crash matrix |
...
| Parent lease expires while colocated child still runs | The child keeps the colocation group alive on its worker; no other worker can claim the parent until the group lease lapses. The parent's old owner is the only worker that can re-acquire the parent without stealing the group. |
| Worker holding a colocated group dies | Member leases and the group lease all expire; a recovery sweep clears them and the next claim re-homes the parent and colocated child together onto a new worker. |
