...
| Timers | `temporal/tests/workflow_timer_test.go`; `sdk-go/internal/internal_command_state_machine_test.go` timer state-machine cases | Timer waits, due wakeups, timer plus event sequences, and DST timer/partition paths already exist. | Kept current coverage. Timer cancellation was rejected because Durababble has no timer-cancel API yet. |
| Signals and workflow commands | `temporal/tests/signal_workflow_test.go`; `temporal/tests/query_workflow_test.go`; SDK update/signal ordering tests | Event waits and fanout existed; workflow `expose_command` was mostly covered as a persistence stub, not an end-to-end wake path. | Added end-to-end exposed-command delivery through the durable event wait used by the current prototype. |
...
| Cancellation / termination | `temporal/tests/cancel_workflow_test.go`; `temporal/tests/workflow_test.go` termination cases | Durababble does not have first-class workflow cancellation/termination semantics. | Rejected as API scope expansion. The imported command-delivery test only asserts current durable command-event delivery. |
| Retries | `sdk-go/internal/common/backoff/retry_test.go`; server `tests/workflow_test.go` retry cases | Step retry policies and restart persistence already existed. | Added durable-object command retry idempotency coverage so the same command operation key survives retry. |
...
| Parallel async execution | SDK goroutine/coroutine and update ordering tests in `sdk-go/test/integration_test.go` | Durababble currently executes workflow orchestration synchronously; DST models concurrent clients/workers outside workflow code. | Rejected workflow-local async import until deterministic workflow fibers are designed. |
...
The ported subset lives in `test/durababble/temporal_inspired_test.rb`:

- Large completed history prefix replay without rerunning completed step bodies.
- Exposed workflow command delivery through persisted event waits.
- Durable object command retry preserving a stable generated idempotency key.

These tests translate Temporal concepts into Durababble's current workflows,
steps, waits, workflow command events, durable objects, and command retry model.
