...
| `[DURABABBLE-LEASE-4]` | `completeStep`, `completeWorkflow`, `completeInboxCommand`, `staleOwnersCannotCommit`, `workflowInboxCommandCommitsNeedWorkflowLease` | `WorkflowExecution#assert_workflow_lease!`, `WorkflowStepRunner` step commit paths, `SqlStore#require_fenced_workflow_update!`, workflow inbox claim/completion fences | `DurababbleHardeningTest`, worker lifecycle, backend conformance workflow inbox lease tests, DST workflow RPC owner matrix | Alloy check UNSAT; tests pass |
...
| `[DURABABBLE-FENCE-1]` | `acquireFence`, `completeFence`, `failFence`, `idempotencyFencesPreventDuplicateSideEffects`, `staleFenceTokensCannotFinish`, `exampleAbandonedFenceRemainsRunningAfterCrashReplay`, `exampleFenceFailureReplaysError` | `PostgresStore#with_fence` plus MySQL equivalent paths | `DurababbleHardeningTest`, backend conformance, DST `fenced_side_effect_once` | Alloy check UNSAT; examples SAT; tests pass |
