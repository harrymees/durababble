    ...
    def migrate!
      ...
      execute(<<~SQL)
        CREATE TABLE IF NOT EXISTS #{table("workflows")} (
          ...
          colocation_group_id VARCHAR(191),
      ...
      execute(<<~SQL)
        CREATE TABLE IF NOT EXISTS #{table("colocation_groups")} (
          ...
          created_at DATETIME(6) NOT NULL DEFAULT NOW(6),
          updated_at DATETIME(6) NOT NULL DEFAULT NOW(6),
          INDEX #{quote_column_name(index_name("colocation_groups", "worker_lease"))} (locked_by),
          INDEX #{quote_column_name(index_name("colocation_groups", "expired_lease"))} (locked_until)
        )
      SQL
      execute(<<~SQL)
