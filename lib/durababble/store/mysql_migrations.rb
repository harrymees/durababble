    ...
    def migrate!
      ...
      execute(<<~SQL)
        CREATE TABLE IF NOT EXISTS #{table("object_sleeps")} (
          ...
          updated_at DATETIME(6) NOT NULL DEFAULT NOW(6),
          PRIMARY KEY (object_type, object_id),
          UNIQUE INDEX #{@connection.quote_column_name(index_name("object_sleeps", "sleep_id"))} (sleep_id),
          INDEX #{@connection.quote_column_name(index_name("object_sleeps", "due"))} (wake_at, updated_at)
        )
      SQL
      @migrated = true
      self
