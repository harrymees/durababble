...
module Durababble
  class MysqlStore < Store
    ...
    #: () -> untyped
    def drop_schema!
      ["object_sleeps", "durable_object_commands", "target_activations", "inbox", "mailbox_sequences", "durable_objects", "waits", "outbox", "fences", "step_attempts", "steps", "workflow_history", "workflows"].each { |name| execute("DROP TABLE IF EXISTS #{table(name)}") }
      @migrated = false
    end
    ...
    def wake_due_object_sleeps(now: Time.now)
      transaction do
        sleeps = execute_params(<<~SQL, [now]).map { |row| decode_row(row) }
          ...
          FOR UPDATE
        SQL
        sleeps.each { |sleep| convert_object_sleep_to_wake_without_transaction(sleep) }
        sleeps.length
      end
    end
    ...
    #: (untyped) -> untyped
    def convert_object_sleep_to_wake_without_transaction(sleep)
      payload = {
        "method_name" => Durababble::DurableObjectRef::WAKE_METHOD_NAME,
        "args" => [],
        "kwargs" => { payload: sleep["payload"] },
      }
      shape_hash = inbox_shape_hash(
        target_kind: "object",
        target_type: sleep.fetch("object_type"),
        target_id: sleep.fetch("object_id"),
        message_kind: "wake",
        method_name: Durababble::DurableObjectRef::WAKE_METHOD_NAME,
        payload:,
      )
      sequence = allocate_mailbox_sequence(target_kind: "object", target_type: sleep.fetch("object_type"), target_id: sleep.fetch("object_id"))
      execute_params(<<~SQL, [sleep.fetch("sleep_id"), sleep.fetch("object_type"), sleep.fetch("object_id"), sequence, sleep.fetch("sleep_id"), shape_hash, dump_serialized(payload)])
        INSERT IGNORE INTO #{table("inbox")} (id, target_kind, target_type, target_id, sequence, message_kind, method_name, operation_id, shape_hash, payload, status)
        VALUES (?, 'object', ?, ?, ?, 'wake', '#{Durababble::DurableObjectRef::WAKE_METHOD_NAME}', ?, ?, ?, 'pending')
      SQL
      set_target_activation_pending_without_transaction(target_kind: "object", target_type: sleep.fetch("object_type"), target_id: sleep.fetch("object_id"), ready_at: Time.now)
