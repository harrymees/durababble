  ...
  class PostgresStore < Store
    ...
    def convert_object_sleep_to_wake_without_transaction(sleep)
      ...
      execute_params(<<~SQL, [sleep.fetch("sleep_id"), sleep.fetch("object_type"), sleep.fetch("object_id"), sequence, sleep.fetch("sleep_id"), shape_hash, dump_serialized(payload)])
        INSERT INTO #{table("inbox")} (id, target_kind, target_type, target_id, sequence, message_kind, method_name, operation_id, shape_hash, payload, status)
        VALUES ($1, 'object', $2, $3, $4, 'wake', '#{Durababble::DurableObjectRef::WAKE_METHOD_NAME}', $5, $6, $7::bytea, 'pending')
