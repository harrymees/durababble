...
module Durababble
  class Store
    ...
    def object_commands_for(object_type:, object_id:)
      inbox_messages_for(target_kind: "object", target_type: object_type, target_id: object_id).map { |row| object_command_row(row) }
    end
    ...
    def object_command_message?(row)
      row && (!row.key?("target_kind") || (row.fetch("target_kind") == "object" && ["ask", "wake"].include?(row.fetch("message_kind"))))
    end
