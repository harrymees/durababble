...
module Durababble
  ...
  # full queue parks the producer fiber (and, for the remote case, gRPC/HTTP2
  # flow control parks the server-side producer too).
  #
  ...
  class ResultStream
    ...
    # for back-pressure; gRPC/HTTP2 flow control bounds bytes on the wire separately.
    STREAM_BUFFER = 16
    ...
    # its own teardown (for a remote producer, cancelling the gRPC stream);
    # we wait for that within a grace window. If the producer ignores
    # cancellation, the grace expires and we hard-stop it, bounding the wait so
    # `close`/teardown cannot deadlock. Off a reactor there is nothing to
