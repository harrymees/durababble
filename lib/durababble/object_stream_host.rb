...
module Durababble
  ...
  # the lease on `(worker_pool, object_type, object_id)`, heartbeat it, and
  # eventually release it. Many in-flight stream RPCs for the same object on
  # this worker share a single lease (the host refcounts per key) so the
  # worker remains the single, exclusive object owner for its lifetime here.
  #
  # The host also runs the renewal loop: a background thread periodically renews
  # every held key via `Store#renew_object_lease`. A renewal that fails flips
  # the entry's `lost` flag; producers see that via `ObjectStreamLeaseWriter`
  # (raises `StaleLease` on emit, and reports `cancelled?` true so indefinite
  # poll-and-emit producers exit). On graceful shutdown the worker calls
  # `evict_all!` so consumers see a terminal `StaleLease` frame rather than a
  # dropped TCP connection.
  #
  # The renewal runs in a Ruby Thread, not an Async fiber, so it survives any
  # individual stream-handler task ending and so its DB connection does not
  # share the reactor's fiber-isolated connection.
  class ObjectStreamHost
    ...
    # Marks the entry for `(worker_pool, object_type, object_id)` as lost and
    # releases the row in the store. Idempotent: returns false if no entry is
    # currently held.
    ...
    # Stops the renewal thread. Called from `WorkerRuntime#stop_rpc_server` on
    # the way out so the host has no live background work after shutdown.
    #: () -> void
    def stop!
      thread = @mutex.synchronize do
        @stopping = true
        renewal = @renewal_thread
        @renewal_thread = nil
        renewal
      end
      thread&.wakeup if thread&.alive?
      thread&.join(@renew_interval + 1.0)
    end
    ...
    def release_lease_safely(key)
      @store.release_object_lease(object_type: key[1], object_id: key[2], worker_id: @worker_id)
    rescue StandardError => err
      Durababble.logger&.warn("ObjectStreamHost: release_object_lease failed for #{key.join("/")}: #{err.class}: #{err.message}")
    end

    # Lazily starts the single renewal thread. Held under `@mutex`.
    #: () -> void
    def ensure_renewal_thread!
