...
module Durababble
  module StoreQueries
    ...
    define(:pg_claim_selected_outbox, backend: :postgres) do |store|
      <<~SQL.chomp
        UPDATE #{table(store, "outbox")}
        SET status = 'processing', locked_by = $2, locked_until = now() + ($3::bigint * interval '1 microsecond')
        WHERE id = $1
        RETURNING *
      SQL
    end
    ...
    define(:mysql_claim_workflow_for_activation_update, backend: :mysql, description: "Claim a workflow through its activation path.") do |store|
      <<~SQL.chomp
        UPDATE #{table(store, "workflows")}
        SET status = 'running', error = NULL, locked_by = ?, locked_until = DATE_ADD(NOW(6), INTERVAL ? MICROSECOND), updated_at = NOW(6)
        WHERE id = ? AND worker_pool = ?
      SQL
    end
